...
#if defined(NDEBUG)
# error "PnDcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6a3d85c37b9b66f2f36217666f681dd020de91c8860ac762723fb9f17108408a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PnDcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PnDcoin Signed Message:\n";
...
{
    // PnDcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 64 * COIN; //You get this many coins per block mined

    // Subsidy is cut in half every 105000 blocks, which will occur approximately every 6 months
    nSubsidy >>= (nHeight / 105000); // PnDcoin: 105k blocks in ~6 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // PnDcoin: 1 day(s) to return back to normal difficulty after no more mining
static const int64 nTargetSpacing = 1 * 60; // PnDcoin: Tries to space blocks out at this time. 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PnDcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PnDcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[3] = 0x4d; //This all spells out SCAM
        hashGenesisBlock = uint256("0xde506564e92a922547638984810ed9537ea4e66b5932f449324ed53194e5c8e3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Onion 2019-07-12 Trump Honors Brave Heroes Who Slept With Wives Of Deployed Soldiers";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 64 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eb802d2a392a885906506ec5cbc3386ac2fa054072aacee0eecd64854a05179552d9d971b5ec9a68e448f3fa15ef31526b8dfced7910ff17238df3ba470b4e2f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1562971621;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085124593;

        if (fTestNet)
        {
            block.nTime    = 1562971592;
            block.nNonce   = 386365491;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd47bf7dbaf3e4c0e8e9bdd82f907bc1452d5223c8051dc7c38010e33f6c1a994"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x73, 0x63, 0x61, 0x6d }; // PnDcoin: this spells out "scam"
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PnDcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PnDcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PnDcoinMiner(CWallet *pwallet)
{
    printf("PnDcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pndcoin-miner");
    ...
    try { loop {
        ...
        printf("Running PnDcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PnDcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PnDcoinMiner, pwallet));
}
